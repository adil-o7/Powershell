function New-PusherPassword {
    <#
    .SYNOPSIS
        Generates a new password as SecureString.
    .PARAMETER Length
        Length of password required. 8 characters by default. Allowed range 4-64 (aliased as -n).
    .PARAMETER Capitals
        Minimum number of capital Latin letters in the resulting password. By default 1/4 of total length
        Setting to 0 will ensure no capital letters are used in password generation. Allowed range 0-64 (aliased as -c).
    .PARAMETER Digits
        Minimum number of digits 0-9 in the resulting password. By default 1/6 of total length.
        Setting to 0 will ensure no numeric characters are used in password generation. Allowed range 0-64 (aliased as -d).
    .PARAMETER Symbols
        Minimum number of symbols in the resulting password. By default 1/8 of total length.
        Setting to 0 will ensure no symbols are used in password generation. Allowed range 0-64 (aliased as -s).
    .PARAMETER Lowers
        Minimum number of lowercase Latin letters in the resulting password. By default 1/2 of total length remaining after allocating caps, digits and symbols with a minimum of 1.
        Setting to 0 will ensure no lowercase letters are used in password generation. Allowed range 0-64 (aliased as -l).
    .PARAMETER Simple
        Sets complexity to off (generates an all-lowercase password) (aliased as a).
          New-PusherPassword -Capitals 0 -Digits 0 -Symbols 0
    .PARAMETER Pin
        Forces generation of an all-numeric PIN (aliased as p).
        New-PusherPassword -Capitals 0 -Lowers 0 -Symbols 0
    .PARAMETER ExcludeHard
        Removes most ambiguous characters from available pool to make sure password is still legible even if written by hand on paper (aliased as h).
    .PARAMETER ExcludeSoft
        ...
        accepted by most applications that have restrictions due to database limitations (aliased as -o).
    .PARAMETER ExcludeChars
        Specifies custom list of characters to exclude from available pool to adapt to specific requirements (aliased as -x).
    .PARAMETER Entropy
        Specifies minimum entropy value. Would be reduced in processing if set above the thoretical maximum. Default value is 3 (max for 8-character string) (aliased as -e).
    .EXAMPLE 
        Generates a random password 8 characters long with full printable charset.
          New-PusherPassword
    .EXAMPLE
        Generates a 16-character lowercase password.
          New-PusherPassword -Length 16 -Simple
          New-PusherPassword -Length 20 -Hard
    #>
    ...
    if (($Lowers + $Capitals + $Digits + $Symbols) -eq 0) {
        Write-Error "New-PusherPassword : Couldn't determine character composition with all 0s requested" -ErrorAction Stop
    }
    ...
    switch ($psCmdlet.ParameterSetName) {
        ...
        Default {
            ...
            if ($Length -lt ($Lowers + $Capitals + $Digits + $Symbols)) {
                ...
                Write-Host -ForegroundColor Yellow "Requested character composition exceeds requested password length, extending to accommodate"
            } else {
    ...
    } while ((ConvertFrom-SecurePassword -SecurePassword $SecPwd | Get-StringEntropy) -lt $Entropy)
